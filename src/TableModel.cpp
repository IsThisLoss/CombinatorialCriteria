...
{
    out << tr("Входная\t\t\tОтклонение\tВид отклонения\n");
    out << tr("последовательность\tот среднего\n");
    for (auto& row : rows)
        out << row.value << "\t\t\t" << row.diff << "\t\t" << row.type << '\n';
    out << '\n';
    out << tr("Среднее арифметическое: ") << stats.mean << '\n';
    out << tr("Количество серий: ") << stats.numberOfSeries << '\n';
    out << "n_1: " << stats.n[0] << " n_2: " << stats.n[1] << " n:" << stats.len << '\n';
    out << "R_min: " << stats.Rmin << " R_max: " << stats.Rmax << '\n';
}
